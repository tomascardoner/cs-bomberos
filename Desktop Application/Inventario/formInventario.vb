...
#Region "Form stuff"
    Friend Sub SetAppearance()
        Me.Icon = CardonerSistemas.Graphics.GetIconFromBitmap(My.Resources.IMAGE_ELEMENTOS_32)
        ...
        Try
            Using dbContext As New CSBomberosContext(True)
                mlistInventarioBase = (From i In dbContext.Inventario
                                       ...
                                       Select New GridRowData With {.IDInventario = i.IDInventario, .IDElemento = e.IDElemento, .IDCuartel = a.IDCuartel, .CuartelNombre = c.Nombre, .IDArea = i.IDArea, .AreaNombre = a.Nombre, .Codigo = a.Codigo & i.Codigo, .Nombre = e.Nombre, .DescripcionPropia = i.DescripcionPropia, .IDUbicacion = If(ug Is Nothing, CardonerSistemas.Constants.FIELD_VALUE_NOTSPECIFIED_SHORT, ug.IDUbicacion), .UbicacionNombre = If(ug Is Nothing, "", ug.Nombre), .IDSubUbicacion = If(sug Is Nothing, CardonerSistemas.Constants.FIELD_VALUE_NOTSPECIFIED_SHORT, sug.IDSubUbicacion), .SubUbicacionNombre = If(sug Is Nothing, "", sug.Nombre), .EsActivo = i.EsActivo}).ToList
            End Using
        ...
        If Not mSkipFilterData Then
            ...
            Try
                ...
                If comboboxUbicacion.SelectedIndex > 0 Then
                    If CShort(comboboxUbicacion.ComboBox.SelectedValue) = CardonerSistemas.Constants.FIELD_VALUE_NOTSPECIFIED_SHORT Then
                        mlistInventarioFiltradaYOrdenada = mlistInventarioFiltradaYOrdenada.Where(Function(e) e.IDUbicacion = CardonerSistemas.Constants.FIELD_VALUE_NOTSPECIFIED_SHORT).ToList
                    Else
                ...
                If comboboxSubUbicacion.SelectedIndex > 0 Then
                    If CShort(comboboxSubUbicacion.ComboBox.SelectedValue) = CardonerSistemas.Constants.FIELD_VALUE_NOTSPECIFIED_SHORT Then
                        mlistInventarioFiltradaYOrdenada = mlistInventarioFiltradaYOrdenada.Where(Function(e) e.IDSubUbicacion = CardonerSistemas.Constants.FIELD_VALUE_NOTSPECIFIED_SHORT).ToList
                    Else
...
#Region "Controls behavior"
    ...
    Private Sub comboboxCuartel_SelectedIndexChanged() Handles comboboxCuartel.SelectedIndexChanged
        pFillAndRefreshLists.Area(comboboxArea.ComboBox, True, False, CByte(comboboxCuartel.ComboBox.SelectedValue), False)
            ...
            If Permisos.VerificarPermiso(Permisos.INVENTARIO_ELIMINAR) Then
                ...
                Using dbContext = New CSBomberosContext(True)
                    ...
                    If MsgBox(Mensaje, CType(MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo, MsgBoxStyle), My.Application.Info.Title) = MsgBoxResult.Yes Then
                        ...
                        Catch ex As Exception
                            CardonerSistemas.ErrorHandler.ProcessError(ex, "Error al eliminar el Elemento en el Inventario.")
                        End Try
