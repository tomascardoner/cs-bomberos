...
CREATE PROCEDURE usp_Persona_ListadoJerarquiaAntiguedad
	...
	BEGIN
		...
		SELECT p.IDPersona, c.Nombre AS CuartelNombre, p.MatriculaNumero, p.ApellidoNombre, ne.IncluyeSecundario AS SecundarioCompleto, 
				ca.Nombre AS CargoNombre, cj.Nombre AS JerarquiaNombre, pa.Fecha AS FechaUltimoAscenso, 
				dbo.PersonaObtenerAntiguedadEnLetras(p.IDPersona, pa.Fecha, @FechaHasta) AS AntiguedadCargoLeyenda, #PersonaOrden.Orden
			FROM Persona AS p
			...
			WHERE p.EsActivo = 1
				...
				AND (pab.IDAltaBaja IS NULL OR pab.IDAltaBaja = dbo.PersonaObtenerIdUltimaAltaBaja(p.IDPersona, @FechaHasta))
				AND (pa.Fecha IS NULL OR pa.Fecha = dbo.PersonaObtenerFechaUltimoAscenso(p.IDPersona, @FechaHasta))
